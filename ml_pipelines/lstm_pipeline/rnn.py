...
def preprocess_data(data):
    ...
    with open('rnn_scaler.pkl', 'wb') as f:
...
# Function 4: Define the Vanilla RNN model
...
def train_model(model, train_loader, num_epochs, criterion, optimizer):
    for epoch in range(num_epochs):
        ...
        with open('rnn_trained_model.pkl', 'wb') as f:
            pickle.dump(model, f)
    return model
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python rnn.py <stock> <input_size> <hidden_size> <num_layers> <num_epochs>")
        sys.exit(1)
    ...
    # Initialize and train the Vanilla RNN model
    model = StockPriceRNN(input_size, hidden_size, num_layers, output_size=1)
